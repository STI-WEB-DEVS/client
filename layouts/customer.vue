<template>
  <div class="min-h-screen bg-gray-50 flex flex-col relative">
    ...
    <div
      ...
      class="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm"
    >
      <div class="w-[90%] max-w-md rounded-3xl bg-white p-8 shadow-2xl border border-gray-200 animate-popup">
      ...
      <div class="w-[90%] max-w-lg rounded-3xl bg-white p-8 shadow-2xl border border-gray-200 animate-popup">
        ...
        <div class="flex gap-3">
          ...
          <button
            ...
            class="flex-1 rounded-2xl bg-green-700 py-3 text-sm font-bold text-white hover:bg-green-800 transition-all disabled:opacity-50"
          >
...
// If your app has user management setup, import your user auth hook here:
...
// CHECKOUT INTEGRATION WITH LARAVEL VALIDATION
const handleCheckout = async () => {
  ...
  try {
    ...
    // 🌟 CONNECTED HERE: Supplying customer_uuid string data to avoid validation drop
    // Replace the fallback string with real logic if your user session is wired up (e.g., auth.user.uuid)
    const activeCustomerUuid = "a1e04141-5063-4803-af69-01b80701342e" 

    const checkoutPayload = {
      customer_uuid: activeCustomerUuid, 
    ...
    // Hits your Backend OrderController@store endpoint
    await orderService.create(checkoutPayload)

    // Reset Frontend UI State on DB success confirmation
    cartItems.value = []
    ...
    console.error('Database connection transaction failure:', err.response?.data || err)
    
    // Read specific message validation strings returned from Laravel's error engine if visible
    const validationMessage = err.response?.data?.message || 'Could not save order. Please try again.'
    triggerNotification(validationMessage)
  } finally {
