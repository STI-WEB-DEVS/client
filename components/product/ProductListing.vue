...
import { ShoppingCartIcon, MagnifyingGlassIcon, FunnelIcon } from '@heroicons/vue/24/outline'
...
const PALETTES = ['bg-rose-50', 'bg-sky-50', 'bg-amber-50', 'bg-violet-50', 'bg-emerald-50', 'bg-orange-50']
function cardBg(index: number) { return PALETTES[index % PALETTES.length] }
</script>
  ...
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div class="relative w-full sm:max-w-xs">
        ...
        <input v-model="search" type="text" placeholder="Search products…" class="w-full rounded-lg border border-gray-200 bg-white py-2 pl-9 pr-4 text-sm text-gray-700 shadow-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
      </div>
      <div class="flex items-center gap-2">
        <FunnelIcon class="h-4 w-4 text-gray-400" />
        <select v-model="sortBy" class="rounded-lg border border-gray-200 bg-white py-2 pl-3 pr-8 text-sm text-gray-700 shadow-sm outline-none transition focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100">
    ...
    <div v-else-if="error" class="flex flex-col items-center gap-3 py-20 text-center">
      ...
      <button class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700" @click="fetchProducts(currentPage)">Retry</button>
    </div>
    ...
    <div v-else-if="filtered.length === 0" class="flex flex-col items-center gap-2 py-20 text-center">
      ...
      <p class="text-sm text-gray-400">{{ search ? 'No products match your search.' : 'No products available yet.' }}</p>
    </div>
    ...
    <div v-else>
      ...
      <div v-if="meta && meta.last_page > 1" class="mt-10 flex items-center justify-center gap-2">
        <button :disabled="currentPage === 1" class="rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-600 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40" @click="fetchProducts(currentPage - 1)">← Prev</button>
        <template v-for="page in meta.last_page" :key="page">
          <button v-if="Math.abs(page - currentPage) <= 2 || page === 1 || page === meta.last_page" :class="['rounded-lg border px-3 py-1.5 text-sm transition', page === currentPage ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-200 text-gray-600 hover:bg-gray-50']" @click="fetchProducts(page)">{{ page }}</button>
          <span v-else-if="Math.abs(page - currentPage) === 3" class="px-1 text-gray-400">…</span>
        </template>
        <button :disabled="currentPage === meta.last_page" class="rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-600 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40" @click="fetchProducts(currentPage + 1)">Next →</button>
      </div>
      <p v-if="meta" class="mt-4 text-center text-xs text-gray-400">Showing {{ products.length }} of {{ meta.total }} products</p>
    </div>

    <!-- Buy Now Modal -->
    <OrderModal v-model="showOrderModal" :product="selectedProduct" />
  </div>
</template>