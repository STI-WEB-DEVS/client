  ...
  <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div v-else-if="error" class="flex flex-col items-center gap-3 py-20 text-center">
      ...
      <button class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-indigo-700"
        @click="fetchProducts(currentPage)">Retry</button>
    </div>
    ...
    <div v-else>
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        <div v-for="(product, index) in filtered" :key="product.uuid"
          ...
          {{ product.description }}
            ...
            <!-- Stocks indicator -->
            <p class="text-xs" :class="product.stocks === 0
      ...
      <div v-if="meta && meta.last_page > 1" class="mt-10 flex items-center justify-center gap-2">
        <button :disabled="currentPage === 1"
          ...
          @click="fetchProducts(currentPage - 1)">← Prev</button>
        <template v-for="page in meta.last_page" :key="page">
          <button
            v-if="Math.abs(page - currentPage) <= 2 || page === 1 || page === meta.last_page"
            :class="['rounded-lg border px-3 py-1.5 text-sm transition', page === currentPage ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-200 text-gray-600 hover:bg-gray-50']"
            @click="fetchProducts(page)">{{ page }}</button>
          <span v-else-if="Math.abs(page - currentPage) === 3" class="px-1 text-gray-400">…</span>
        </template>
        <button :disabled="currentPage === meta.last_page"
          class="rounded-lg border border-gray-200 px-3 py-1.5 text-sm text-gray-600 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40"
          @click="fetchProducts(currentPage + 1)">Next →</button>
      </div>
