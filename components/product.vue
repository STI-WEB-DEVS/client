            ...
            >
              <td>
                <div class="client-cell">
                  <div class="avatar">
                    {{ product.name.split(' ').map((w: string) => w[0]).slice(0, 2).join('').toUpperCase() }}
                  </div>
                  <p class="client-name">{{ product.name }}</p>
                </div>
              </td>
              <td class="td-price">{{ formatPrice(product.price) }}</td>
              <td class="td-actions">
                <div class="row-actions">
                  <button class="act-btn edit" @click="openEdit(product)">Edit</button>
                  <span class="act-sep">|</span>
                  <button class="act-btn del" @click="openDelete(product)">Remove</button>
                </div>
              </td>
            </tr>
          </tbody>
        ...
        <div class="modal">
          ...
          <form @submit.prevent="handleCreate" class="modal-form">
            <div class="field">
              ...
              <input v-model="createForm.name" type="text" placeholder="e.g. Linen Blazer" required class="field-input" />
            </div>
            <div class="field">
              <label class="field-label">Price <span class="req">*</span></label>
              <input v-model="createForm.price" type="number" min="0" step="0.01" placeholder="0.00" required class="field-input" />
            </div>
      ...
      >
        <div class="modal">
          ...
          <form @submit.prevent="handleUpdate" class="modal-form">
            <div class="field">
              <label class="field-label">Product Name <span class="req">*</span></label>
              <input v-model="editForm.name" type="text" placeholder="e.g. Linen Blazer" required class="field-input" />
            </div>
            <div class="field">
              <label class="field-label">Price <span class="req">*</span></label>
              <input v-model="editForm.price" type="number" min="0" step="0.01" placeholder="0.00" required class="field-input" />
            </div>
    ...
    <Transition name="fade">
      ...
      >
        <div class="modal modal-sm">
          ...
          <div class="modal-divider"></div>
          <p class="del-warn">
            <strong>{{ selectedProduct?.name }}</strong> will be permanently removed.
...
const createForm = ref({ name: "", price: 0 });
...
.products-table td { padding: 0.85rem 1.4rem; vertical-align: middle; }
...
.td-price { font-size: 0.8rem; color: #9ca3af; }
