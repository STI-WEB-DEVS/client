<template>
  <NuxtLayout>
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      ...
      <button @click="openAdd" class="btn-amber">+ Add Member</button>
    </div>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
      <div v-for="member in team" :key="member.id" class="member-card">
        <div class="member-avatar" :style="{background: avatarColor(member.name)}">{{ initials(member.name) }}</div>
        ...
        <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between">
          ...
          <div class="flex gap-1">
            <button @click="editMember(member)" class="action-btn text-amber-600 hover:bg-amber-50">Edit</button>
      ...
      <div class="modal-box">
        ...
        <form @submit.prevent="saveMember" class="p-6 space-y-4">
          ...
          <div class="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" class="btn-amber">{{ modalMode==='edit'?'Update Member':'Add Member' }}</button>
          </div>
        </form>
      </div>
    </div>
...
const form = reactive({ name:'', username:'', email:'', phone:'', website:'', companyName:'' })
...
const openAdd = () => {
  Object.assign(form, { name:'', username:'', email:'', phone:'', website:'', companyName:'' })
  modalMode.value='add'; showModal.value=true
}
const editMember = (m:any) => {
  selectedMember.value=m
  Object.assign(form,{ name:m.name, username:m.username, email:m.email, phone:m.phone, website:m.website, companyName:m.company?.name||'' })
...
const saveMember = () => {
  if(modalMode.value==='add') {
    team.value.push({ id:nextId++, name:form.name, username:form.username, email:form.email, phone:form.phone, website:form.website, company:{ name:form.companyName } })
  } else {
    const idx = team.value.findIndex(x=>x.id===selectedMember.value?.id)
    if(idx!==-1) {
      Object.assign(team.value[idx],{ name:form.name, username:form.username, email:form.email, phone:form.phone, website:form.website, company:{ name:form.companyName } })
    }
  }
  closeModals()
}
