<template>
  <NuxtLayout>
    <div class="dashboard">
      ...
      <div class="page-header">
        <div class="header-left">
          <div class="header-eyebrow">
            ...
            <span class="eyebrow-label">Customer Management</span>
          </div>

          <h1 class="page-title">Customers</h1>
          <p class="page-sub">
            {{ today }} · Manage and track all customer records
          </p>
        </div>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          <div class="stat-body">
            ...
            <p class="stat-change stat-up">All registered users</p>
          </div>
        </div>
        ...
        <div class="stat-card">
          <div class="stat-body">
            ...
            <p class="stat-change stat-up">Based on search</p>
          </div>
        </div>

        <div class="stat-card stat-card--critical">
          <div class="stat-body">
            <p class="stat-label">Recently Added</p>
            <p class="stat-value">{{ recentCount }}</p>
            <p class="stat-change stat-down">Last 7 days</p>
          </div>
        </div>
      </div>
      ...
      <div class="dark-card">
        <div class="card-head">
          <h2 class="card-title">Search Customers</h2>
        </div>
            ...
            <tbody>
              ...
              >
                <td class="p-4 flex items-center gap-3">
                  <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-red-600 to-red-900 flex items-center justify-center text-white font-bold">
                ...
                <td class="p-4 text-right space-x-3">
                  <button class="text-blue-400" @click="openViewModal(customer)">View</button>
                  <button class="text-green-400" @click="openEditModal(customer)">Edit</button>
                  <button class="text-red-400" @click="openDeleteConfirm(customer)">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>

          <div v-if="filteredCustomers.length === 0" class="p-10 text-center text-white/40">
            No customers found
          </div>
        </div>
      </div>
      ...
      <!-- ═══════════════════════════════
           MODALS (unchanged logic, styled container only)
      ═══════════════════════════════ -->
        ...
        <div class="dark-card w-full max-w-md">
          ...
          <form @submit.prevent="saveCustomer" class="p-6 space-y-4">
            ...
            <div class="flex gap-2 pt-4">
              <button type="button" @click="closeModal" class="flex-1 p-2 border border-white/10 text-white/70 rounded">
                Cancel
              </button>
              <button type="submit" class="flex-1 p-2 bg-red-600 text-white rounded">
                {{ saving ? 'Saving...' : 'Save' }}
              </button>
            </div>
          </form>
      ...
      <div v-if="showViewModal" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
        <div class="dark-card w-full max-w-md p-6 text-white">
          ...
          <p><b>Joined:</b> {{ formatDate(selectedCustomer?.created_at) }}</p>

          <button @click="closeViewModal" class="mt-6 w-full p-2 bg-white/10 rounded">
            Close
          </button>
        </div>
      </div>
      ...
      <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
        <div class="dark-card w-full max-w-sm p-6 text-white">
          <h3 class="text-lg font-bold mb-3">Delete Customer</h3>

          <p class="text-white/60">
            Delete <b>{{ selectedCustomer?.name }}</b>?
          </p>

          <div class="flex gap-2 mt-6">
            <button @click="closeDeleteConfirm" class="flex-1 p-2 border border-white/10 rounded">
              Cancel
            </button>
            <button @click="deleteCustomer" class="flex-1 p-2 bg-red-600 rounded">
              Delete
            </button>
          </div>
        </div>
