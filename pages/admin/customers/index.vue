...
const fetchCustomers = async () => {
  try {
    ...
    // Fixed data mapping logic for Nuxt $fetch + Laravel Resources
    if (res && Array.isArray(res.data)) {
      // Handled: Laravel's standard resource wrap or pagination wrap { data: [...] }
      customers.value = res.data;
    } else if (Array.isArray(res)) {
      // Handled: Clean fallback if an un-wrapped array is returned directly
      customers.value = res;
    } else {
      customers.value = [];
    }
    ...
    setNotification("Failed to load customers.", true);
  }
};
