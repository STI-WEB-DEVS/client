...
onMounted(async () => {
  try {
    ...
    error.value = err
    showFeedback(err?.message || 'Failed to load products')
  } finally {
    pending.value = false
  }
})
...
const handleSave = async (payload: any) => {
  try {
    if (mode.value === 'create') {
      ...
      products.value.data.unshift(res?.data ?? res)

      showFeedback('Product created successfully!')
    } else {
      const res = await productService.update(selectedProduct.value.uuid, payload)

      const i = products.value.data.findIndex(
        (p: any) => p.uuid === selectedProduct.value.uuid
      )

      if (i !== -1) products.value.data[i] = res?.data ?? res
