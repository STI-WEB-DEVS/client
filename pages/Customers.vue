    ...
    <NuxtLayout>
      ...
      <div class="md:flex md:items-center md:justify-between mb-8">
        ...
        <div class="mt-4 flex md:ml-4 md:mt-0 gap-3">
          <button 
            v-if="isAdding || selectedCustomers.length > 0"
            type="button" 
            @click="resetState"
          ...
          <button 
            v-if="selectedCustomers.length > 0 && !isAdding"
            type="button" 
        ...
        <div class="px-4 py-5 sm:p-6">
          <div class="grid grid-cols-6 gap-6">
            <div class="col-span-6 sm:col-span-3">
              ...
              <input v-model="newCustomer.name" type="text" placeholder="John Doe" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm border p-2 outline-none">
            </div>
  
            <div class="col-span-6 sm:col-span-3">
              <label class="block text-sm font-medium text-gray-700">Email Address</label>
              <input v-model="newCustomer.email" type="email" placeholder="john@example.com" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm border p-2 outline-none">
            </div>
  
            <div class="col-span-6 sm:col-span-2">
              <label class="block text-sm font-medium text-gray-700">Status</label>
              <select v-model="newCustomer.status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm border p-2 outline-none">
          ...
          <div class="mt-6 flex items-center justify-end gap-x-4 border-t pt-4">
            <button @click="resetState" type="button" class="text-sm font-semibold leading-6 text-gray-900">Cancel</button>
            <button @click="saveCustomer" type="button" class="rounded-md bg-green-900 px-6 py-2 text-sm font-semibold text-white shadow-sm hover:bg-green-800 transition">Save Customer</button>
          </div>
        </div>
      </div>
            ...
            <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
              <table class="min-w-full divide-y divide-gray-300">
                <thead class="bg-gray-50">
                  <tr>
                    <th scope="col" class="relative px-7 sm:w-12 sm:px-6">
                      <input type="checkbox" class="absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-green-900 focus:ring-green-900" :checked="isAllSelected" @change="toggleSelectAll" />
                    </th>
                    <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">Customer Name</th>
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                  <tr v-for="customer in customers" :key="customer.id" :class="[selectedCustomers.includes(customer.id) && 'bg-green-50']">
                    <td class="relative px-7 sm:w-12 sm:px-6">
                      <input type="checkbox" class="absolute left-4 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-green-900 focus:ring-green-900" :value="customer.id" v-model="selectedCustomers" />
                    </td>
                    ...
                    <td class="whitespace-nowrap px-3 py-4 text-sm">
                      <span :class="[customer.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800', 'inline-flex items-center rounded-full px-2 py-1 text-xs font-medium']">
                        {{ customer.status }}
                      </span>
                    </td>
